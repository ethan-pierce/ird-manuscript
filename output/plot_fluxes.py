...
predicted_sed_yield = fit.slope * np.log10(ice_yield) + fit.intercept
R2 = fit.rvalue**2
print('R^2: ', R2)

plt.scatter(np.log10(ice_yield), np.log10(sed_yield), s = 100, alpha = 0.5)
plt.plot(np.log10(ice_yield), predicted_sed_yield, color = 'black')
...
GrISdf['Ice Discharge'] = [gate_discharge[str(int(i))].iloc[872:2696].mean() for i in GrISdf['Gate']]
